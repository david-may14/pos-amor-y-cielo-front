...
interface NavItem {
  to: string
  label: string
  /** Sin valor, lo ve cualquiera. 'SUPERVISOR' lo ven supervisor y admin. */
  minRol?: 'SUPERVISOR' | 'ADMIN'
...
const navItems: NavItem[] = [
  ...
  },
  {
    to: '/inventario',
    label: 'Inventario',
    // El supervisor registra mermas en piso; exigir al admin para eso es lo
    // que hace que las mermas no se registren.
    minRol: 'SUPERVISOR',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7.979 5.917A9 9 0 1 0 15.953 19M15 6.343A9 9 0 0 1 19 10" />
      </svg>
    ),
  },
  {
...
export default function Layout() {
  const { user, isAdmin, puedeSupervisar, logout, bloquear, hayPin } = useAuth()
  ...
  const visibleItems = navItems.filter((item) => {
    if (!item.minRol) return true
    return item.minRol === 'ADMIN' ? isAdmin : puedeSupervisar
  })
